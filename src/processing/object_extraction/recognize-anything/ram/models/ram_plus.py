...
class RAM_plus(nn.Module):
    ...
    def generate_tag_openset(self,
        ...
        targets = torch.where(
            torch.sigmoid(logits) > torch.tensor(threshold).to(image.device),
            torch.tensor(1.0).to(image.device),
            torch.zeros(self.num_class).to(image.device)
        )
        ...
        for b in range(bs):
            ...
            for idx in indices:
                tag_name = str(self.tag_list[idx]) 
                confidence = float(probs[b][idx])
                sample_tags.append((tag_name, confidence))
            
            tag_output.append(sample_tags)
